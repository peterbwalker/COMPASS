...
export default function Globe3D({ apiKey, nodes, routes, highlightedRouteIds = [] }) {
  ...
  const routePolylinesRef = useRef([]); // [{ element, baseColor, isPulsing }]
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Cinematic entrance: start zoomed way out (like a satellite view of
      // the whole hemisphere) and descend into the operating area, rather
      // than snapping straight to the final framing.
      const TARGET_RANGE = 900000;
      const START_RANGE = 12000000;
      ...
      animateEntrance(map, START_RANGE, TARGET_RANGE, 20, 55, 2400);
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const route of routes) {
        ...
        const isThreatened = route.threat_spike || route.status === "closed";
        ...
        if (isThreatened) {
          routePolylinesRef.current.push({
            element: line,
            baseWidth: isHighlighted ? 6 : 3,
          });
        }
      }

      if (routePolylinesRef.current.length > 0) {
        startPulseLoop(routePolylinesRef, pulseFrameRef);
      }
    })();
...
/**
 * Animates Map3DElement's range/tilt from a starting value to a target
 * value over `durationMs`, using requestAnimationFrame with an ease-out
 * curve. Map3DElement exposes range/tilt/center as plain settable
 * properties, so re-assigning them each frame re-renders the view --
 * there's no built-in "flyTo" transition on this element, so we drive it
 * manually.
 */
function animateEntrance(map, startRange, endRange, startTilt, endTilt, durationMs) {
  ...
  function step(now) {
    ...
    map.range = startRange + (endRange - startRange) * eased;
    map.tilt = startTilt + (endTilt - startTilt) * eased;
...
/**
 * Continuously pulses the strokeWidth of contested/closed route polylines
 * so active threats visually stand out rather than sitting static. Runs
 * until the next redraw cancels it (see the cleanup in the routes effect).
 */
function startPulseLoop(polylinesRef, frameRef) {
  const startTime = performance.now();

  function step(now) {
    const elapsed = (now - startTime) / 1000;
    // Sine wave oscillation between ~0.7x and ~1.3x the base stroke width.
    const factor = 1 + 0.3 * Math.sin(elapsed * 2.5);

    for (const { element, baseWidth } of polylinesRef.current) {
      element.strokeWidth = baseWidth * factor;
    }
